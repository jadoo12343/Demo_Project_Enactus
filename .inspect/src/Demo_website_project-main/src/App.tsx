...
import { ArrowDown, ArrowUpRight, Check, ChevronDown, Clock3, Instagram, Linkedin, Mail, MapPin, Menu, MoveUpRight, Sparkles, X } from 'lucide-react';
...
function Nav() {
  ...
  return <header className={scrolled ? 'site-header scrolled' : 'site-header'}><ScrollProgress /><div className="nav-inner"><Logo /><nav className={open ? 'nav-links is-open' : 'nav-links'} aria-label="Main navigation">{links.map(([label, href]) => <a href={href} key={href} onClick={() => setOpen(false)}>{label}</a>)}<a className="nav-cta" href="#register" onClick={() => setOpen(false)}>Get your pass <ArrowUpRight size={15} /></a></nav><button className="menu-button" ref={menuRef} onClick={() => setOpen(!open)} aria-label={open ? 'Close menu' : 'Open menu'} aria-expanded={open}>{open ? <X /> : <Menu />}</button></div></header>;
}
...
function Footer() {
  return <footer className="site-footer"><div className="footer-cta"><Reveal><h2 className="footer-headline">See you at <em>Innovision.</em></h2><a className="button button-primary" href="#register">Get your pass <ArrowUpRight size={17} /></a></Reveal></div><div className="footer-top"><Logo /><p>Two days. One campus.<br /><span>Infinite ways forward.</span></p><a className="footer-mail" href={`mailto:${event.email}`}>{event.email} <ArrowUpRight size={15} /></a></div><div className="footer-bottom"><span>© 2027 Innovision, NSUT</span><div className="socials"><a href={event.socials.instagram} aria-label="Instagram" rel="noopener noreferrer"><Instagram size={17} /></a><a href={event.socials.linkedin} aria-label="LinkedIn" rel="noopener noreferrer"><Linkedin size={17} /></a><a href={event.socials.facebook} aria-label="Facebook" rel="noopener noreferrer"><span className="facebook-icon">f</span></a></div><span>Built as an Enactus R2 Tech Task prototype.</span></div></footer>;
}
