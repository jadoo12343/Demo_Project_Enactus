...
function Gallery() {
  ...
  return <section className="gallery-section" id="moments"><div className="section-intro"><Reveal><p className="eyebrow">Moments</p><h2>What it<br /><em>feels like.</em></h2></Reveal><Reveal delay={150}><p>A glimpse of the energy, the people, and the ideas that fill two days at NSUT.</p></Reveal></div><div className="marquee" aria-label="Photo gallery"><div className="marquee-track">{duplicated.map((photo, i) => <a className="marquee-item" key={i} href={`/events/${schedule[i % schedule.length].id}`} aria-label={`View event details for ${photo.caption}`}><img src={photo.url} alt={photo.caption} loading="lazy" /><div className="marquee-overlay"><span className="marquee-tag">{photo.tag}</span><span className="marquee-caption">{photo.caption}</span><span className="marquee-open">View event <ArrowUpRight size={14} /></span></div></a>)}</div></div></section>;
}
