...
function Nav() {
  ...
  return <header className={scrolled ? 'site-header scrolled' : 'site-header'}><ScrollProgress /><div className="nav-inner"><Logo /><nav className={open ? 'nav-links is-open' : 'nav-links'} aria-label="Main navigation">{links.map(([label, href]) => <a href={href} key={href} onClick={() => setOpen(false)}>{label}</a>)}<a className="nav-cta" href="#register" onClick={() => setOpen(false)}>Get your pass to register now <ArrowUpRight size={15} /></a></nav><button className="menu-button" ref={menuRef} onClick={() => setOpen(!open)} aria-label={open ? 'Close menu' : 'Open menu'} aria-expanded={open}>{open ? <X /> : <Menu />}</button></div></header>;
}
